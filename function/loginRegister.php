<?php ob_start(); ?>
<!--untuk menyimpan klo ada output dihalamannya-->
...
//mysqli_connect digunakan untuk membuka koneksi baru ke server mySQL.
...
function acak($panjang)
{
    ...
    $string = '';
    for ($i = 0; $i < $panjang; $i++) {
        $pos = rand(0, strlen($karakter) - 1); //rand menghasilkan integer acak, strlen mengembalikan panjang string
        $string = $karakter[$pos];
    }
    return $string;
}
...
function register($dataRegister)
{
    ...
    //stripcslashes untuk menghilangkan garis miring terbalik \.
    ...
    //mysqli_real_escape_string untuk membuat string SQL legal yang dapat digunakan dalam pernyataan SQL
    $passwordConfirm = mysqli_real_escape_string($koneksi, htmlspecialchars($dataRegister['password-confirm']));

    //untuk cek apakah ada username atau email yg sudah dipakai
    $cekUser = mysqli_query($koneksi, "SELECT email, username FROM users WHERE email = '$email' OR username = '$username'");
    //Mysqli_query berfungsi untuk menjalankan instruksi ke mysql.
    ...
    //password_hash untuk enkripsi password agar password lebih kuat.
